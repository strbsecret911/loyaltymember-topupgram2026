/* =========================================
   BAGIAN 1: KONFIGURASI FIREBASE & IMPORT
   ========================================= */
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";
import { 
  getFirestore, doc, getDoc, setDoc, updateDoc, collection, addDoc, getDocs, query, where, limit, serverTimestamp 
} from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
import { 
  getAuth, GoogleAuthProvider, signInWithPopup, signOut, onAuthStateChanged 
} from "https://www.gstatic.com/firebasejs/10.13.0/firebase-auth.js";

// Konfigurasi dari file firebase.js kamu
...
function go(hash){
  location.hash = hash;
  setTimeout(render, 0); // Pastikan render ulang
}
...
/* ===== Ticket JPG via Canvas ===== */
function drawTicketToCanvas(canvas, {
  brand="Topupgram Loyalty",
  name="-",
  telegramId="-",
  memberCode="-",
  voucher="-",
  ...
  const accent = "#C9DBF0";
  const text = "#474B59";
  const muted = "rgba(71,75,89,.75)";
  const border = "#e6e8ef";

  ctx.fillStyle = bg;
  ctx.fillRect(0,0,W,H);

  const x=60, y=60, w=W-120, h=H-120, r=32;
  ...
  ctx.fillText("Tiket Voucher", x+44, y+86);

  ctx.fillStyle = muted;
  ctx.font = "600 28px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillText(brand, x+44, y+128);
  ...
  ctx.save();
  ctx.fillStyle = "rgba(201,219,240,.35)";
  ...
  ctx.fillText("KODE VOUCHER", bx+34, by+58);

  ctx.fillStyle = text;
  ctx.font = "900 68px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  const vw = ctx.measureText(String(voucher)).width;
  ctx.fillText(String(voucher), bx + (bw - vw)/2, by+150);

  ctx.fillStyle = muted;
  ctx.font = "600 22px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillText("Tunjukkan kode ini saat order", bx+34, by+208);

  ctx.fillStyle = muted;
  ctx.font = "600 26px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillText(`Redeem: ${pointsUsed} poin`, left, y + h - 120);

  ctx.fillStyle = text;
  ctx.font = "800 30px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillText(`Selamat! Kamu mendapat potongan Rp${Number(discount).toLocaleString("id-ID")}`, left, y + h - 78);

  ctx.fillStyle = muted;
  ctx.font = "600 24px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillText(`Waktu redeem: ${whenText}`, left, y + h - 34);
}

function downloadCanvasJPG(canvas, filename="voucher-topupgram.jpg"){
  try{
...
function wirePublicRedeemButtons(member){
  document.getElementById("recheck").onclick = ()=>{ location.hash="#/"; render(); };

  document.querySelectorAll("button[data-redeem]").forEach(btn=>{
    btn.onclick = ()=> publicRedeem(member, btn.getAttribute("data-tid"), Number(btn.getAttribute("data-redeem")));
  });
}

async function publicRedeem(member, telegramId, pointsToUse){
    ...
    `Redeem ${pointsToUse} poin untuk Rp${discount.toLocaleString("id-ID")}?\nPoin akan berkurang.`,
    {
      ...
      onOk: async ()=>{
        ...
        wrap.querySelector("#p_ok").onclick = async ()=>{
          ...
          const whenText = fmtDateTimeNow();

          openModal("Redeem Berhasil", "Voucher kamu sudah dibuat. Lihat tiket di bawah.", {
            okText:"OK",
            onOk: async ()=>{
              ...
              const safeTid = telegramId;
              const safeMemberCode = m.member_code || makeMemberCode(telegramId);

              resultBox.insertAdjacentHTML("afterbegin", `
                <div class="card accent" id="ticket_wrap">
                  <div class="small"><b>Tiket Voucher (JPG)</b></div>
                  <canvas id="ticket_canvas" style="width:100%; border-radius:16px; margin-top:10px;"></canvas>

                  <div class="row" style="margin-top:10px;">
                    <button class="btn" id="dl_ticket">Download JPG</button>
                    <button class="btn secondary" id="copy_v">Salin Kode</button>
                  </div>

                  <div class="mini" style="margin-top:8px;">
                    Kode Voucher: <b>${escapeHtml(voucher)}</b>
                  </div>
                </div>
              `);

              const canvas = document.getElementById("ticket_canvas");
              drawTicketToCanvas(canvas, {
                name: safeName,
                telegramId: safeTid,
                memberCode: safeMemberCode,
                voucher,
              ...
              document.getElementById("copy_v").onclick = async ()=>{
                try{
              ...
              document.getElementById("dl_ticket").onclick = ()=>{
                const ok = downloadCanvasJPG(canvas, `voucher-${voucher}.jpg`);
                if(!ok){
                  try{
                    const url = canvas.toDataURL("image/jpeg", 0.92);
                    window.open(url, "_blank");
                  }catch{
                    openModal("Info", "Gagal membuat JPG di perangkat ini.", { okText:"OK" });
                  }
                }
              };
...
function pageAdmin(){
  ...
  function loginView(){
    ...
    `;
    document.getElementById("a_back").onclick = ()=> go("#/");
  ...
  function adminView(user){
    ...
    document.getElementById("a_logout").onclick = adminLogout;

    const t_add = document.getElementById("t_add");
    const t_points = document.getElementById("t_points");
  ...
  function adminTabs(){
    ...
    if(t_add) t_add.classList.toggle("active", tab==="add");
    if(t_points) t_points.classList.toggle("active", tab==="points");
  ...
  function adminTabAdd(){
    ...
    const m_tid = document.getElementById("m_tid");
    const m_name = document.getElementById("m_name");
    const m_user = document.getElementById("m_user");
  ...
  function adminTabPoints(){
    ...
    document.getElementById("p_check").onclick = async ()=> adminShowMember();
    document.getElementById("p5").onclick = async ()=> adminAddPoints(5);
    document.getElementById("p10").onclick = async ()=> adminAddPoints(10);
    document.getElementById("p25").onclick = async ()=> adminAddPoints(25);

    const p_tid = document.getElementById("p_tid");
    const p_out = document.getElementById("p_out");

    document.getElementById("reset_pin").onclick = async ()=>{
...
window.addEventListener("load", render);
render();
```0  const exp = m.expires_at?.toDate ? m.expires_at.toDate() : (m.expires_at ? new Date(m.expires_at) : null);
  if(!exp) return { label:"-", expired:false, exp:null };
  const expired = (new Date() > exp);
  return { label: expired ? "KEDALUWARSA" : "AKTIF", expired, exp };
}
